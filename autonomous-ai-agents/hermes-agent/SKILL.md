...
| `provider_routing` | OpenRouter-only provider selection knobs (`sort`, `only`, `ignore`, `order`, `data_collection`) |
| `checkpoints` | `enabled`, `max_snapshots` (50) |

Copilot-specific note: if `model.provider: copilot`, use `smart_model_routing` to steer simple turns to a cheaper Copilot model. `provider_routing` is for OpenRouter and will not help Copilot request optimization.
